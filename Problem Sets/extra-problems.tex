...
\subsection{Man Pulling}
A painter of mass \( M \) stands on a scaffold of mass \( m \) and pulls himself up by two ropes which hang over pulleys, as shown in the figure.  
He pulls each rope with force \( F \) and accelerates upward with a uniform acceleration \( a \). Find \( a \), neglecting the fact that no one could do this for long.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.5\linewidth]{misc/KK2-man-pulling.png}
    \caption{Enter Caption}
    \label{fig:enter-label}
\end{figure}

There are 4 sections of rope pulling up with tension $\mathbf{T} = \mathbf{F}$.

So there are $4\mathbf{F}$ pulling up. We should have the following expression
\[
4\mathbf{F} - (M+m)\mathbf{g} = (M+m)\mathbf{a}
\]
Solve for $\mathbf{a}$, we have
\[
\mathbf{a} = \frac{4\mathbf{F} - (M+m)\mathbf{g}}{M+m}
\]
Where $M$is the mass of the scaffolding, and $m$ is the mass of the man. 

And if $\mathbf{F} = M\mathbf{g}$, we see that $\mathbf{a} = \mathbf{g}$.
